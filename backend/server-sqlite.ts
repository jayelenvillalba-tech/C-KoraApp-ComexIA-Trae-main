...
// Initialize Ably for Real-Time Chat
import * as Ably from 'ably';
let _ably: Ably.Rest | null = null;
export const getAbly = () => {
  if (!_ably) {
    _ably = new Ably.Rest({ 
      key: process.env.ABLY_API_KEY || 'dummy:key',
      log: { level: 1 } 
    });
  }
  return _ably;
...
// Phase Sprint 2: HS Codes Route
import hsCodesRouter from './routes/hs-codes';
app.use('/api/hs-codes', hsCodesRouter);

// Phase Sprint 2: Companies Route
import companiesRouter from './routes/companies';
app.use('/api/companies', companiesRouter);

// Phase Sprint 2: Market Analysis Route
...
// Sprint 2: Periodic WAL Checkpoint (every 24h) to prevent .wal bloat
setInterval(() => {
  try {
    if (sqliteDb) {
      console.log('🧹 [DB] Running PRAGMA wal_checkpoint(TRUNCATE)...');
      sqliteDb.exec('PRAGMA wal_checkpoint(TRUNCATE);');
    }
  } catch (error) {
    console.error('❌ [DB] Error in WAL checkpoint:', error);
  }
}, 24 * 60 * 60 * 1000);
...
// [NEW] Market Analysis Endpoint (Moved to backend/routes/market-analysis.ts)
...
// ========== HS Codes API (Moved to backend/routes/hs-codes.ts) ==========

// ========== Country Recommendations API (Moved to backend/routes/country-recommendations.ts) ==========

// ========== Companies API (Moved to backend/routes/companies.ts) ==========
...
// ========== Market Analysis API (Historical/Docs Moved to backend/routes/market-analysis.ts) ==========
...
app.get('/api/market-analysis/recommendations', async (req, res) => {
  try {
    ...
    const allTreaties = [
      ...
      { country: 'Alemania', countryCode: 'DE', treaty: 'TLC Unión Europea (Pendiente)', rank: 4, coordinates: [51.1657, 10.4515] },
    ];
    const treatyRecommendations = allTreaties
      .filter(t => t.countryCode !== origin)
      .slice(0, 3)
      .map((t, i) => ({ ...t, rank: i + 1 }));
    ...
    let cheComexRecommended = activePosts
      .filter((p: any) => p.country && getCountryCode(p.country) !== origin)
    ...
    const allBuyers = [
      ...
      { country: 'India', countryCode: 'IN', rank: 4, volume: 4200000, avgValue: 450, details: { landedCost: 500, tariff: 5.0 } },
    ];
    const topBuyers = allBuyers
      .filter(b => b.countryCode !== origin)
      .slice(0, 3)
      .map((b, i) => ({ ...b, rank: i + 1 }));
    ...
    const enrichedPosts = await Promise.all(
      posts.map(async (post) => {
        ...
        return {
          ...
          user: user[0] || null,
          requirements: parseArray(post.requirements),
          certifications: parseArray(post.certifications)
        };
      })
    );
...
app.get('/api/marketplace/posts/:id', async (req, res) => {
  try {
    ...
    res.json({
      ...
      company: company[0] || null,
      user: user[0] || null,
      requirements: parseArray(post[0].requirements),
      certifications: parseArray(post[0].certifications)
    });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
// Get company profile (Moved to backend/routes/companies.ts)
...
// Ably Token Auth
app.get('/api/chat/ably-auth', async (req, res) => {
  if (!process.env.ABLY_API_KEY || process.env.ABLY_API_KEY === 'not_configured') {
    return res.status(503).json({ error: 'Ably no configurado en el servidor.' });
  }
  
  const clientId = (req.query.userId as string) || 'anonymous';
  
  try {
    const tokenRequest = await getAbly().auth.createTokenRequest({ clientId });
...
app.post('/api/chat/conversations/:id/messages', async (req, res) => {
  try {
    ...
    if (process.env.ABLY_API_KEY && process.env.ABLY_API_KEY !== 'not_configured') {
      try {
        const channel = getAbly().channels.get(`conversation:${id}`);
...
// ========== PHASE 21: ADVANCED MARKETPLACE ENDPOINTS ==========

// GET /api/marketplace/trade-blocks - Get all trade blocks with member countries
app.get('/api/marketplace/trade-blocks', async (req, res) => {
  try {
    const { tradeBlocks } = await import('../shared/trade-blocks');
    res.json(tradeBlocks);
  } catch (error: any) {
    console.error('Error fetching trade blocks:', error);
    res.status(500).json({ error: error.message });
  }
});

// GET /api/marketplace/tariff-preferences - Calculate trade preferences
app.get('/api/marketplace/tariff-preferences', async (req, res) => {
  try {
    const { hsCode, originCountry, destinationCountry } = req.query;
    
    if (!hsCode || !originCountry || !destinationCountry) {
      return res.status(400).json({ 
        error: 'Missing required parameters: hsCode, originCountry, destinationCountry' 
      });
    }

    const { calculateTradePreferences } = await import('../shared/trade-blocks');
    const preferences = calculateTradePreferences(
      hsCode as string,
    ...
    console.error('Error calculating tariff preferences:', error);
    res.status(500).json({ error: error.message });
  }
});

// Enhanced GET /api/marketplace/posts with Phase 21 filters
app.get('/api/marketplace/posts/enhanced', async (req, res) => {
  try {
    const { 
      sector, 
      type, 
    ...
    // Trade block filter
    if (tradeBlock) {
      const { getCountriesByBlock } = await import('../shared/trade-blocks');
      const blockCountries = getCountriesByBlock(tradeBlock as string);
      
      if (blockCountries.length > 0) {
        const countryConditions = blockCountries.map(c => 
          or(
            eq(marketplacePosts.originCountry, c),
            eq(marketplacePosts.destinationCountry, c)
          )
        );
    ...
    // Fetch posts with company data
    let posts = await db
      .select({
        post: marketplacePosts,
        company: companies
      })
      .from(marketplacePosts)
    ...
    console.error('Error fetching enhanced marketplace posts:', error);
    res.status(500).json({ error: error.message });
  }
});
...
(async () => {
  try {
    ...
    // Ensure cache tables exist (exchange rates, tariffs, comtrade, wto)
    sqliteDb.prepare(`
    ...
    `).run();
    console.log('[startup] ✅ Tablas de caché (exchange, tariff, comtrade, wto) verificadas.');
