...
export default function PostForm({ onClose, onSubmit }: PostFormProps) {
  ...
  // Phase 23: AI Classification State
  ...
  const handleAIClassification = async () => {
    ...
    try {
      ...
      if (data.hsCode) {
        ...
        // Auto-fill if high confidence and field is empty
        if (data.confidence > 0.8 && !formData.hsCode) {
          updateField("hsCode", data.hsCode);
        }
      }
    } catch (error) {
                ...
                <div>
                  ...
                  <div className="mt-2 flex justify-end">
                    <Button
                      ...
                      disabled={isClassifying || !formData.productName}
                      className="text-xs text-blue-300 hover:text-blue-200 hover:bg-blue-900/20"
                    >
                      {isClassifying ? (
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {language === 'es' ? 'Sugerir HS Code con IA' : 'Categorize with AI'}
                        </span>
                      )}
                    </Button>
                  </div>
                  ...
                  {showAiSuggestions && aiSuggestions.length > 0 && (
                    <div className="mt-2 p-2 bg-slate-900/90 border border-blue-500/30 rounded-lg space-y-2">
                      <div className="flex items-center justify-between text-xs text-gray-400 px-1">
                        <span>{language === 'es' ? 'Sugerencias IA:' : 'AI Suggestions:'}</span>
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          className="h-4 p-0 text-gray-500 hover:text-white"
                      ...
                      {aiSuggestions.map((suggestion, idx) => (
                        <button
                          key={idx}
                          type="button"
                          onClick={() => applyAiSuggestion(suggestion.code)}
                        ...
                        >
                          ...
                          <p className="text-xs text-gray-400 group-hover:text-gray-300 line-clamp-1">
