...
export const marketData = sqliteTable("market_data", {
  ...
  sourceApi: text("source_api").default("manual"), // 'comtrade', 'worldbank', 'manual'
});
...
export const marketplacePosts = sqliteTable("marketplace_posts", {
  ...
  // Smart Filters Fields
  sector: text("sector"), // e.g. 'Agriculture', 'Technology'
  subcategory: text("subcategory"), // e.g. 'Soybeans', 'Software'
  postType: text("post_type").default("buy"), // 'buy' | 'sell' | 'social_post' | 'social_event'
  incoterm: text("incoterm"), // e.g. 'FOB', 'CIF'
  price: real("price"),
  currency: text("currency").default("USD"),
  isEcological: integer("is_ecological", { mode: "boolean" }).default(false),
  status: text("status").default("active"), // 'active' | 'closed'
  createdAt: integer("created_at", { mode: "timestamp" }).$defaultFn(() => new Date()),
  expiresAt: integer("expires_at", { mode: "timestamp" }),
  // Phase 21: Advanced Features
  descriptionLong: text("description_long"), // Alibaba-style detailed description
  photos: text("photos"), // JSON array of photo URLs
  moq: text("moq"), // Minimum Order Quantity
  inspectionAvailable: integer("inspection_available", { mode: "boolean" }).default(false),
  regionalContentPercentage: integer("regional_content_percentage"), // For MERCOSUR preferences
  tradePreferences: text("trade_preferences"), // JSON array of applicable agreements
});
